...
import (
	...
	"github.com/erda-project/erda/modules/pipeline/pkg/task_extensions"
...
func getValueByKey(pipelineID uint64, key string) interface{} {
...
func getStagesCachesFromContextByPipelineID(pipelineID uint64) []spec.PipelineStage {
	value := getValueByKey(pipelineID, PipelineStagesContextCachesPrefixKey)
	if value == nil {
		return nil
	}
...
func getPipelineYmlCachesFromContextByPipelineID(pipelineID uint64) *pipelineyml.PipelineYml {
	value := getValueByKey(pipelineID, PipelineYmlContextCachesPrefixKey)
	if value == nil {
		return nil
	}
...
func getPipelineRerunSuccessTasksFromContextByPipelineID(pipelineID uint64) map[string]*spec.PipelineTask {
	value := getValueByKey(pipelineID, PipelineRerunSuccessTaskContextCachesPrefixKey)
	if value == nil {
		return nil
	}
...
func getPassedDataWhenCreateFromContextByPipelineID(pipelineID uint64) *task_extensions.PassedDataWhenCreate {

	value := getValueByKey(pipelineID, PipelinePassedDataWhenCreateContextCachesPrefixKey)
	if value == nil {
		return nil
	}
	passedData, ok := value.(*task_extensions.PassedDataWhenCreate)
	if !ok {
		return nil
	}
...
func setPassedDataWhenCreateToContextByPipelineID(passedDataWhenCreate *task_extensions.PassedDataWhenCreate, pipelineID uint64) {
	cacheMap.Store(makeMapKey(pipelineID, PipelinePassedDataWhenCreateContextCachesPrefixKey), passedDataWhenCreate)
}

// get value from caches map
// if not exist search value from db and save to caches map
func getOrSetPassedDataWhenCreateFromContext(bdl *bundle.Bundle, pipelineYml *pipelineyml.PipelineYml, pipelineID uint64) (passedDataWhenCreate *task_extensions.PassedDataWhenCreate, err error) {
	...
	passedDataWhenCreate = &task_extensions.PassedDataWhenCreate{}
