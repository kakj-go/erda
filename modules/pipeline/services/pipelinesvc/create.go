...
func (s *PipelineSvc) createPipelineGraphStage(p *spec.Pipeline, pipelineYml *pipelineyml.PipelineYml, txSession *xorm.Session) (stages []*spec.PipelineStage, err error) {
	...
	for si := range pipelineYml.Spec().Stages {
		...
		if err := s.dbClient.CreatePipelineStage(ps, dbclient.WithTxSession(txSession)); err != nil {
...
func (s *PipelineSvc) MergePipelineYmlTasks(pipelineYml *pipelineyml.PipelineYml, dbTasks []spec.PipelineTask, p *spec.Pipeline, dbStages []spec.PipelineStage, passedDataWhenCreate *task_extensions.PassedDataWhenCreate) (mergeTasks []spec.PipelineTask, err error) {
...
func (s *PipelineSvc) getYmlActionTasks(pipelineYml *pipelineyml.PipelineYml, p *spec.Pipeline, dbStages []spec.PipelineStage, passedDataWhenCreate *task_extensions.PassedDataWhenCreate) []spec.PipelineTask {
...
func (s *PipelineSvc) CreatePipelineGraph(p *spec.Pipeline) (err error) {
	...
	_, err = s.dbClient.Transaction(func(session *xorm.Session) (interface{}, error) {
		...
		stages, err = s.createPipelineGraphStage(p, pipelineYml, session)
		if err != nil {
			return nil, err
		}
